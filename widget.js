(function() {
  ...
  var SHOP_ID = script.getAttribute('data-shop-id') || 'SHOP_001';
  var COLOR = '#4F46E5';

  // Charge la config de la boutique
  fetch(SHOPBOT_URL + '/config/' + SHOP_ID)
    .then(function(r) { return r.json(); })
    .then(function(config) {
      COLOR = config.color || '#4F46E5';
      init(config.name);
    })
    .catch(function() { init('Support'); });

  function init(shopName) {
    ...
    style.innerHTML = `
      #shopbot-btn {
        ...
        font-size: 26px; box-shadow: 0 4px 20px rgba(0,0,0,0.2);
      ...
      #shopbot-header { background: ${COLOR}; color: white; padding: 16px 20px; display: flex; align-items: center; gap: 12px; }
      .sb-avatar { width: 38px; height: 38px; border-radius: 50%; background: rgba(255,255,255,0.2); display: flex; align-items: center; justify-content: center; font-size: 20px; }
      ...
      #shopbot-msgs { flex: 1; overflow-y: auto; padding: 16px; display: flex; flex-direction: column; gap: 10px; }
      .sb-bot { max-width: 80%; padding: 10px 14px; border-radius: 16px; font-size: 13px; line-height: 1.5; background: #f0f0f0; color: #333; align-self: flex-start; border-bottom-left-radius: 4px; }
      .sb-user { max-width: 80%; padding: 10px 14px; border-radius: 16px; font-size: 13px; line-height: 1.5; background: ${COLOR}; color: white; align-self: flex-end; border-bottom-right-radius: 4px; }
      ...
      #shopbot-send { width: 38px; height: 38px; background: ${COLOR}; border: none; border-radius: 50%; color: white; font-size: 18px; cursor: pointer; }
    ...
    win.innerHTML = `
      <div id="shopbot-header">
        ...
        <div class="sb-info">
          <p>Assistant ${shopName}</p>
          <small>● En ligne · Répond instantanément</small>
        </div>
        <button id="shopbot-close">✕</button>
      </div>
      <div id="shopbot-msgs">
        <div class="sb-bot">Bonjour ! Je suis l'assistant de ${shopName}. Comment puis-je vous aider ? 😊</div>
      </div>
      <div id="shopbot-sugg">
    ...
    document.getElementById('shopbot-send').onclick = envoyer;
    document.getElementById('shopbot-input').onkeydown = function(e) { if(e.key === 'Enter') envoyer(); };

    window.shopbotSuggest = function(t) {
      document.getElementById('shopbot-input').value = t;
      envoyer();
    };
    ...
    async function envoyer() {
      ...
      um.className = 'sb-user'; um.innerText = texte;
      ...
      try {
        var res = await fetch(SHOPBOT_URL + '/chat', {
          ...
          body: JSON.stringify({ message: texte, shopId: SHOP_ID })
        });
        var data = await res.json();
        typing.remove();
        var bm = document.createElement('div');
        bm.className = 'sb-bot'; bm.innerText = data.reply;
        msgs.appendChild(bm); msgs.scrollTop = msgs.scrollHeight;
      } catch(e) {
        typing.remove();
        var bm2 = document.createElement('div');
        bm2.className = 'sb-bot'; bm2.innerText = 'Désolé, une erreur est survenue.';
